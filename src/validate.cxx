...
void validate_user_input(Prob& problem, Alg& algorithm, Workspace* workspace)
{
    ...
    if (algorithm.nlp_method != "IPOPT" && algorithm.nlp_method != "SNOPT" )
       error_message("Incorrect NLP method specified. The only valid values are \"IPOPT\" or \"SNOPT\" ", problem.outdir.c_str());
    if (algorithm.collocation_method != "Legendre" && algorithm.collocation_method!="Chebyshev" && algorithm.collocation_method!="trapezoidal" && algorithm.collocation_method!="Hermite-Simpson")
       error_message("Incorrect collocation method specified. Valid options are \"Legendre\" , \"Chebyshev\", \"trapezoidal\", and \"Hermite-Simpson\" ", problem.outdir.c_str());
    if (algorithm.scaling != "automatic" && algorithm.scaling!="user")
       error_message("Incorrect scaling option specified. Valid options are \"automatic\" and \"user\" ", problem.outdir.c_str());
    if (algorithm.defect_scaling != "state-based" && algorithm.defect_scaling!="jacobian-based")
       error_message("Incorrect differential defect scaling option specified. Valid options are \"state-based\" and \"jacobian-based\" ", problem.outdir.c_str());
    if (algorithm.derivatives != "automatic" && algorithm.derivatives!="numerical")
       error_message("Incorrect derivatives option specified. Valid options are \"automatic\" and \"numerical\" ", problem.outdir.c_str());
    if (algorithm.hessian != "exact" && algorithm.hessian!="limited-memory")
       error_message("Incorrect algorithm.hessian option specified. Valid options are \"limited-memory\" and \"exact\" ", problem.outdir.c_str());
    ...
    if (algorithm.diff_matrix != "standard" && algorithm.diff_matrix!="diff_matrix" && algorithm.diff_matrix!="central-differences" &&  algorithm.diff_matrix!="reduced-roundoff" )
       error_message("Incorrect algorithm.diff_matrix option specified. Valid options are \"standard\", \"reduced-roundoff\" , \"central-differences\" ", problem.outdir.c_str());
    ...
    if (algorithm.nlp_tolerance <= 0)
       error_message("algorithm.nlp_tolerance must be positive", problem.outdir.c_str());
    if (algorithm.nlp_iter_max <= 0)
       error_message("algorithm.iter_max must be positive", problem.outdir.c_str());

    if (algorithm.nsteps_error_integration <= 0)
       error_message("algorithm.nsteps_error_integration must be positive", problem.outdir.c_str());

    if (algorithm.ode_tolerance <= 0)
       error_message("algorithm.ode_tolerance must be positive", problem.outdir.c_str());

    if (algorithm.mr_max_increment_factor <= 0 || algorithm.mr_max_increment_factor > 1 )
       error_message("algorithm.mr_max_increment_factor must be in the interval (0,1]", problem.outdir.c_str());
    if (algorithm.mr_max_iterations <= 0)
       error_message("algorithm.mr_max_iterations must be positive", problem.outdir.c_str());
    if (algorithm.mr_min_extrapolation_points < 2)
       error_message("algorithm.mr_min_extrapolation_points must be >= 2", problem.outdir.c_str());

    if (algorithm.mr_initial_increment <= 0)
       error_message("algorithm.mr_initial_increment must be positive", problem.outdir.c_str());

    if (algorithm.mr_kappa <= 0 || algorithm.mr_kappa>1.0 )
       error_message("algorithm.mr_kappa must be in the interval (0,1]", problem.outdir.c_str());

    if (algorithm.mr_M1 <= 0  )
       error_message("algorithm.mr_M1 must be positive", problem.outdir.c_str());

    if (algorithm.switch_order < 0  )
       error_message("algorithm.mr_M1 must be >= 0", problem.outdir.c_str());

    if (algorithm.mesh_refinement != "automatic" &&  algorithm.mesh_refinement != "manual" )
       error_message("algorithm.mesh_refinement must either \"manual\" or \"automatic\" ", problem.outdir.c_str());
    ...
    for (i=0;i<problem.nphases;i++)
    {
 		...
 		error_message(m, problem.outdir.c_str());
         }
